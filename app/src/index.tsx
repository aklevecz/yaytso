...
function ContextProvider({ children }: { children: React.ReactChild }) {
    ...
    useEffect(() => {
        ...
        const rpcAddress =
            process.env.NODE_ENV === "development"
                ? "http://localhost:8545"
                : `https://${networkName}.infura.io/v3/${INFURA}`;

        const provider = new ethers.providers.JsonRpcProvider(rpcAddress);

        let contractAddress = YAYTSO_MAIN_ADDRESS;

        if (chainId !== 1) {
            contractAddress = YaytsoInterface.networks[NETWORK_ID].address;
        }
        const contract = new ethers.Contract(
            contractAddress,
            YaytsoInterface.abi,
        ...
        if (window.ethereum) {
            ...
            window.ethereum.on("networkChanged", (networkId: any) => {
                ...
                if (networkId !== "1") {
                    contractAddress = (YaytsoInterface.networks as any)[
                        networkId
                    ].address;
                } else {
                ...
                setContract(contract);
                getSignerAndAddress(provider).then(
